  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (type === "like" || type === "comment") {
        if (!postId) throw ApiError.badRequest("Can't create notification :(");

        const post = await prisma?.post.findUnique({ where: { id: postId } });

        if (!post) throw ApiError.badRequest("Can't create notification :(");
        ...
        if (type === "like") {
          notification = await prisma?.notification.create({
            data: {
              ...
              } liked your post "${post.body.slice(0, 30)}..."`,
              image: post.image,
          ...
          notification = await prisma?.notification.create({
            data: {
              body: `User ${
                currentUser?.username
              } wrote comment to your post "${post.body.slice(0, 30)}..."`,
              username: post.username,
            },
          });
        }
